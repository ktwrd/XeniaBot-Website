# Xenia Bot Guide

Work-in-progress. If you have any questions or you're confused, please don't hesitate [to join our server](https://discord.gg/UMzjTrC2U8) and ask some questions.

- [Modules](/guide/modules)
    - [About Moderation](/guide/about_moderation)
        - [About BanSync](/guide/about_bansync)
        - [Warn System](/guide/about_warns)
        - [About Role Preservation](/guide/about_rolepreserve) - A feature to restore a users roles when they re-join a server.
    - [Required Permissions](/guide/required_permissions) - Glossary of all the required permissions for all modules.

You can find at a comprehensive list of Xenia's commands on: [discordbotlist.com](https://discordbotlist.com/bots/xenia/commands)